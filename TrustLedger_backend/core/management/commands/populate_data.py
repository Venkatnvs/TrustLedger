...
class Command(BaseCommand):
    help = 'Populate initial data for TrustLedger'

    def handle(self, *args, **options):
        self.stdout.write('Creating initial data for TrustLedger...')
        ...
        for source_data in fund_sources_data:
            ...
            self.stdout.write(f"{'Created' if created else 'Found'} fund source: {source.name}")
        ...
        for project_data in projects_data:
            ...
            self.stdout.write(f"{'Created' if created else 'Found'} project: {project.name}")
        ...
        for flow_data in fund_flows_data:
            ...
            self.stdout.write(f"{'Created' if created else 'Found'} fund flow: {flow}")
        ...
        for indicator_data in trust_indicators_data:
            ...
            self.stdout.write(f"{'Created' if created else 'Found'} trust indicator for {indicator.department.name}: {indicator.overall_score}/100")
        ...
        for metric_data in impact_metrics_data:
            ...
            self.stdout.write(f"{'Created' if created else 'Found'} impact metric: {metric}")
        
        self.stdout.write(
            self.style.SUCCESS(
                f'\nInitial data population completed successfully!\n'
                f'Created {len(departments)} departments\n'
                f'Created {len(fund_sources)} fund sources\n'
                f'Created {len(projects)} projects\n'
                f'Created {len(fund_flows_data)} fund flows\n'
                f'Created {len(trust_indicators_data)} trust indicators\n'
                f'Created {len(impact_metrics_data)} impact metrics'
            )
        )